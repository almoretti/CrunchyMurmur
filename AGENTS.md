...
`site/` contains the public landing page, deployed to Railway as its own project (service `site`, deployed by running `railway up` inside `site/`). It deliberately depends on repository facts. When you change any of the following, update `site/` in the same change:
